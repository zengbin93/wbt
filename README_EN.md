...
- wb.to_result(target_vol=0.20) → BacktestResult: the standard input object for plotting and the strategy-review page (see "Plotting" below). Normalized curves, history/recent verdicts and yearly alpha share target_vol, yearly_days and full-sample ddof=0. Undefined normalized curves contain NaN (null on export); the legacy `long_alpha_stats` property keeps its fixed 0.20 target.
