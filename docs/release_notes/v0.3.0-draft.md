...
All `wbt.plotting` functions now accept `result: BacktestResult`:
`plot_cumulative_returns(result, keys=...)`, `plot_drawdown(result, key=...)`,
`plot_daily_return_dist`, `plot_monthly_heatmap`, `plot_symbol_returns`,
`plot_pairs_analysis`, `plot_backtest_overview`, `plot_colored_table`,
`plot_long_short_comparison`, plus new `plot_key_trades`, `plot_drawdowns_table`,
`plot_verdict`.
...
plot_colored_table(result)                                       # was: (wb.stats)
plot_long_short_comparison(result)                               # was: (daily_return, stats, long, short)
```
